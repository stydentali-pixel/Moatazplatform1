...
export default function IdeasAdmin() {
  ...
  async function scheduleIdea(id: string) {
    const dateStr = prompt("أدخل تاريخ ووقت النشر (YYYY-MM-DD HH:mm):", new Date(Date.now() + 86400000).toISOString().slice(0, 16).replace('T', ' '));
    if (!dateStr) return;
    
    const r = await fetch(`/api/admin/ideas/${id}/convert`, {
      method: "POST", credentials: "include", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ status: "SCHEDULED", scheduledAt: dateStr }),
    });
    const j = await r.json();
    if (j?.success) router.push(`/admin/posts/${j.data.id}`);
    else alert(j?.error || "فشل الجدولة");
  }
      ...
      <div className="grid md:grid-cols-[1fr,380px] gap-6">
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-3 mt-3 text-xs text-ink-500">
                    <span className={`px-2 py-1 rounded-full font-bold ${
                      ...
                      i.status === "SCHEDULED" ? "bg-purple-100 text-purple-800" :
                      "bg-cream-200 text-ink-700"
                    }`}>{STATUS_AR[i.status]}</span>
                    <span>{new Date(i.createdAt).toLocaleDateString("ar-EG")}</span>
                    {i.source ? <span className="opacity-70 font-bold text-gold-700">· مصدر: {i.source === 'telegram' ? 'تليجرام 🤖' : 'يدوي ✍️'}</span> : null}
                  </div>
                </div>
                <div className="flex flex-col gap-2 text-[11px]">
                  {i.status === "SUGGESTED" || i.status === "APPROVED" ? (
                    <button onClick={() => convertToDraft(i.id)} className="px-3 py-1.5 rounded-lg bg-gold-700 text-cream-50 hover:bg-gold-800 transition-colors">تحويل لمسوّدة</button>
                  ) : null}
                  {i.status === "SUGGESTED" ? (
                    <>
                      <button onClick={() => setStatus(i.id, "APPROVED")} className="px-3 py-1.5 rounded-lg bg-emerald-700 text-cream-50 hover:bg-emerald-800 transition-colors">اعتماد</button>
                      <button onClick={() => setStatus(i.id, "REJECTED")} className="px-3 py-1.5 rounded-lg bg-red-100 text-red-700 hover:bg-red-200 transition-colors">رفض</button>
                    </>
                  ) : null}
                  {i.status === "APPROVED" ? (
                    <>
                      <button onClick={() => publishIdea(i.id)} className="px-3 py-1.5 rounded-lg bg-ink-900 text-cream-50 hover:bg-ink-800 transition-colors">نشر مباشر</button>
                      <button onClick={() => scheduleIdea(i.id)} className="px-3 py-1.5 rounded-lg bg-purple-700 text-cream-50 hover:bg-purple-800 transition-colors">جدولة النشر</button>
                    </>
                  ) : null}
                  <button onClick={() => onDelete(i.id)} className="px-3 py-1.5 rounded-lg bg-cream-100 text-red-700 hover:bg-red-50 transition-colors">حذف</button>
                </div>
              </div>
            </div>
          ))}
        ...
        <form onSubmit={onAdd} className="card p-6 h-fit" data-testid="idea-form">
          ...
          <label className="block mb-3">
            ...
            <input value={title} onChange={(e) => setTitle(e.target.value)} required className="w-full bg-cream-50 border border-ink-900/10 rounded-xl px-4 py-2 text-sm focus:ring-1 focus:ring-gold-500 outline-none" data-testid="idea-title-input"/>
          </label>
          <label className="block mb-4">
            <span className="block text-xs text-ink-500 mb-1">وصف مختصر</span>
            <textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={4} className="w-full bg-cream-50 border border-ink-900/10 rounded-xl px-4 py-2 text-sm focus:ring-1 focus:ring-gold-500 outline-none" data-testid="idea-desc-input"/>
          </label>
          <button className="btn-gold w-full text-sm py-2 disabled:opacity-50" disabled={loading} data-testid="idea-add-btn">
            {loading ? "جاري الإضافة..." : "إضافة الفكرة"}
          </button>
          <p className="text-[11px] text-ink-500 mt-3 leading-5">
            💡 يمكنك أيضاً إضافة الأفكار عبر بوت التليجرام باستخدام أمر <code>/draft</code> أو <code>/ideas</code> للمتابعة.
          </p>
        </form>
      </div>
    </div>
